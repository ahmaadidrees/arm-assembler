...
struct parse_node_st * parse_instruction(struct parse_table_st *pt,
    ...
    if(tp0->id == TK_DIR && tp1->id == TK_IDENT) { 
        ...
        strncpy(np->stmt.inst.label, tp1->value, SCAN_TOKEN_LEN);
        scan_table_accept_any_n(st, 2);
        np->type = DIR;
        return np;
    
    } else if (tp0->id == TK_IDENT && tp1->id == TK_COLON) {
    ...
    if ((opcode == OC_DP) && (tp1->id == TK_IDENT) && 
        ...
        (tp2->id == TK_COMMA) && (tp3->id == TK_IDENT)){
        /* Parse dp2 instructions: dp reg, reg*/
        ...
        /* Parse dp2i instructions: dp reg, imm*/
        ...
        (tp2->id == TK_COMMA) && (tp3->id == TK_IDENT)){
        /* Parse cmp instructions: cmp reg, reg*/
        ...
        (tp2->id == TK_COMMA) && (tp3->id == TK_IMM)){
        /* Parse cmpi instructions: cmp reg, reg*/
        ...
        (tp2->id == TK_COMMA) && (tp3->id == TK_IDENT) && 
        (tp4->id == TK_COMMA) && (tp5->id == TK_IMM)){
        /* Parse lsli instructions: lsl reg, reg, imm*/
  
        np->stmt.inst.type = LSI;
        strncpy(np->stmt.inst.name, tp0->value, SCAN_TOKEN_LEN);
        np->stmt.inst.lsli.rd = parse_reg_to_int(tp1->value);
        np->stmt.inst.lsli.rm = parse_reg_to_int(tp3->value);
        np->stmt.inst.lsli.imm = atoi(tp5->value);
        ...
        np->stmt.inst.mem3.rd = parse_reg_to_int(tp1->value);
        np->stmt.inst.mem3.rn = parse_reg_to_int(tp4->value);
        np->stmt.inst.mem3.rm = atoi(tp6->value);
        ...
        /* Parse b instructions: b char type */
        np->stmt.inst.type = B;
        strncpy(np->stmt.inst.name, tp0->value, SCAN_TOKEN_LEN);
